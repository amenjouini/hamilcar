document.addEventListener("DOMContentLoaded", function () {
  ...
  //  document.querySelectorAll('[data-translatable]').forEach(p => {
  //   const lang = 'fr'; // or 'en' or your detected language
  //   let text = p.getAttribute(`data-${lang}`);
  ...
  switchers.forEach((switcher, idx) => {
    const images = switcher.querySelectorAll('.switch-client-image');
    let currentIndex = 0;
  ...
  // for the technology image switcher
  ...
  if (imageSwitcher) {
    ...
    prevBtn.addEventListener("click", () => {
      if (currentIndex > 0) {
        // Only go prev if not on first image
    ...
    nextBtn.addEventListener("click", () => {
      if (currentIndex < images.length - 1) {
        // Only go next if not on last image
        showImage(currentIndex + 1);
      }
    });
